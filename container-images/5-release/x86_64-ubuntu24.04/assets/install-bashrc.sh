...
# export PARAVIEW_PATH=$(spack location -i paraview+qt) # installed outside spack

if [[ $IMAGE_TARGET =~ "oneapi" ]]; then
  export TAU_SPEC="tau+level_zero+dyninst"
elif [[ $IMAGE_TARGET =~ "cuda" ]] ; then
  export TAU_SPEC="tau+cuda+dyninst"
elif [[ $IMAGE_TARGET =~ "rocm" ]] ; then
  export TAU_SPEC="tau+rocm+dyninst"
else
  export TAU_SPEC="tau~cuda~rocm+dyninst"
fi
...
module use $SPACK_CORE_ROOT/share/spack/modules/linux-ubuntu24.04-x86_64_v3
module use ~/spack/share/spack/modules/linux-ubuntu24.04-x86_64_v3
