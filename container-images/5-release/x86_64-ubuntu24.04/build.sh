...
generic_base_image=${RUNNER_SOURCE_IMAGE:-$UBUNTU_X86_64_RUNNER_IMAGE}
cuda_base_image=${CUDA_MPI_SOURCE_IMAGE:-$UBUNTU_X86_64_CUDA_MPI_IMAGE}
rocm_base_image=${ROCM_MPI_SOURCE_IMAGE:-$UBUNTU_X86_64_ROCM_MPI_IMAGE}
oneapi_base_image=${ONEAPI_MPI_SOURCE_IMAGE:-$UBUNTU_X86_64_ONEAPI_MPI_IMAGE}
cpu_base_image=${CPU_MPI_SOURCE_IMAGE:-$UBUNTU_X86_64_CPU_MPI_IMAGE}

case $target in
  "cuda"*)
    runtime="cuda"
    arch=$(echo $target | cut -d- -f2)
    name=e4s-${runtime}-${arch_tag}
  ...
  "rocm"*)
    ...
    name=e4s-${runtime}-${arch_tag}
    tag=${runtime}${arch}-${common_tag}
    final_base_image=$rocm_base_image
    ;;
  "oneapi")
    runtime="oneapi"
    name=e4s-${runtime}-${arch_tag}
    tag=${common_tag}
    final_base_image=$oneapi_base_image
    ;;
  "cpu")
    runtime="cpu"
    arch=$(echo $target | cut -d- -f2)
...
if [[ $target =~ "cuda" ]]; then
  if mkdir $lockdir; then
    py_tarball=ml-1-20260523-0532-ubuntu2404-py312-cu130.tgz
    cmd wget -q -c -P $download_dir https://cache.e4s.io/$py_tarball
    rm -rf $lockdir
  fi
elif [[ $target =~ "rocm" ]]; then
  if mkdir $lockdir; then
    py_tarball=ml-1-20260607-0818-ubuntu2404-x86_64-py312-rocm723.tgz
    cmd wget -q -c -P $download_dir https://cache.e4s.io/$py_tarball
    rm -rf $lockdir
  fi
elif [[ $target =~ "oneapi" ]]; then
  if mkdir $lockdir; then
    py_tarball=ml-1-20260615-1433-ubuntu2404-x86_64-py312-intel-xpu.tgz
    cmd wget -q -c -P $download_dir https://oaciss.nic.uoregon.edu/e4s/images/26.06/artifacts/$py_tarball
    rm -rf $lockdir
  fi
elif [[ $target =~ "cpu" ]]; then
  if mkdir $lockdir; then
    py_tarball=ml-1-20260606-1453-ubuntu2404-x86_64-py312-cpu.tgz
    cmd wget -q -c -P $download_dir https://cache.e4s.io/$py_tarball
    rm -rf $lockdir
  fi
fi
...
cmd docker build \
 ...
 --build-arg ONEAPI_BASE_IMAGE="$oneapi_base_image" \
