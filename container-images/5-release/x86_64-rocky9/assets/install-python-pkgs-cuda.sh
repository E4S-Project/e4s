...
uv pip install \
 ...
 --system \
 --overrides overrides.txt \
 $wheeldir/nvidia_pytriton-0.7.0*.whl
...
# curl --silent -O https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh
# bash ./Miniconda3-latest-Linux-x86_64.sh -b -p /usr/local/miniconda3
# rm -f Miniconda3-latest-Linux-x86_64.sh
# conda install -c conda-forge libstdcxx-ng=13
# find /usr/local/miniconda3 -type f -name 
wget -q https://cache.e4s.io/$(python3 -c "import urllib.parse; print(urllib.parse.quote('''libstdc++.so.6.0.32'''))")
mv libstdc++.so.6.0.32 /lib64
