...
generic_base_image=${RUNNER_SOURCE_IMAGE:-$UBUNTU_PPC64LE_RUNNER_IMAGE}
cuda_base_image=${CUDA_MPI_SOURCE_IMAGE:-$UBUNTU_PPC64LE_CUDA_MPI_IMAGE}
cpu_base_image=${CPU_MPI_SOURCE_IMAGE:-$UBUNTU_PPC64LE_CPU_MPI_IMAGE}
...
if [[ $target =~ "cuda" ]]; then
  if mkdir $lockdir; then
    py_tarball=ml-ubuntu20.04-ppc64le-cuda-2.tgz
